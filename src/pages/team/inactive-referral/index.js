...
import { teamDirectPendingReferral } from "src/store/apps/team/teamSlice";
...
const InActiveReferral = () => {
  const dispatch = useDispatch();
  const inActiveReferralData = useSelector(
    (state) => state?.team?.teamDirectPendingReferral
  );
  const { tokenBlnc: oneUSDC } = useGetUSDCTokens(1);
  const [inActiveReferral, setInActiveReferral] = useState(null);
  ...
  const formattedData = inActiveReferral
    ? inActiveReferral?.data?.map((item, index) => ({
      id: index + 1 + inActiveReferral?.paginate?.offset,
      serialno: index + 1 + inActiveReferral?.paginate?.offset,
      userid:
  ...
  useEffect(() => {
    if (
      inActiveReferralData &&
      inActiveReferralData?.data &&
      inActiveReferralData?.data?.length > 0
    ) {
      setInActiveReferral(inActiveReferralData);
    }
  }, [inActiveReferralData]);
  ...
  return (
    <Card>
      <CardHeader
        title={`IN ACTIVE REFERRAL COMMUNITY : $${oneUSDC && inActiveReferral?.totalBussiness ? kgcToUSDC(inActiveReferral?.totalBussiness, oneUSDC) : 0}`}
      ...
      <DataGrid
        ...
        rowCount={inActiveReferral?.paginate?.totalItems || 0}
      />
    </Card>
  );
};

export default InActiveReferral;
