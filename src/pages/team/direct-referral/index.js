...
import { teamDirectReferral } from "src/store/apps/team/teamSlice";
...
const DirectReferral = () => {
  ...
  const directReferralData = useSelector(
    (state) => state?.team?.teamDirectReferral
  );
  const { tokenBlnc: oneUSDC } = useGetUSDCTokens(1);
  const [directReferral, setDirectReferral] = useState(null);
  ...
  const formattedData = directReferral
    ? directReferral?.data?.map((item, index) => ({
        id: index + 1 + directReferral?.paginate?.offset,
        serialno: index + 1 + directReferral?.paginate?.offset,
        userid:
          item?.user?._id?.slice(0, 6) + "..." + item?.user?._id?.slice(-6),
        username: item?.user?.userName,
        userrank: item?.userRankStars ? item?.userRankStars : 0 ,
        regdate: item?.latestStakeDate ? new Date(item?.latestStakeDate)?.toLocaleDateString() : 'N/A',
  ...
  useEffect(() => {
    if (
      directReferralData &&
      directReferralData?.data &&
      directReferralData?.data?.length > 0
    ) {
      setDirectReferral(directReferralData);
    }
  }, [directReferralData]);
  ...
  return (
    <Card>
      <CardHeader
        title={`REFERRAL COMMUNITY : $${oneUSDC && directReferral?.totalBussiness ? kgcToUSDC(directReferral?.totalBussiness, oneUSDC) : 0}`}
      ...
      <DataGrid
        ...
        rowCount={directReferral?.paginate?.totalItems || 0}
      />
    </Card>
  );
};
