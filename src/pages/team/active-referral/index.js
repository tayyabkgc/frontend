...
import { teamDirectActiveReferral } from "src/store/apps/team/teamSlice";
...
const ActiveReferral = () => {
  ...
  const activeReferralData = useSelector(
    (state) => state?.team?.teamDirectActiveReferral
  );
  ...
  const formattedData = activeReferral
    ? activeReferral?.data?.map((item, index) => ({
        id: index + 1 + activeReferral?.paginate?.offset,
        serialno: index + 1 + activeReferral?.paginate?.offset,
        userid:
...
}, [dispatch, paginationModel.page, paginationModel.pageSize, searchQuery]);

useEffect(() => {
  if (activeReferralData) {
    setActiveReferral(activeReferralData);
  }
}, [activeReferralData]);
  ...
  return (
    <Card>
      <CardHeader
  title={`DIRECT REFERRAL COMMUNITY : $${oneUSDC && activeReferral?.totalBussiness ? kgcToUSDC(activeReferral?.totalBussiness, oneUSDC) : 0}`}
...
/>
      ...
      <DataGrid
        ...
        rowCount={activeReferral?.paginate?.totalItems || 0}
      />
    </Card>
  );
};
